...
export const createSale = async (db: Firestore, input: Record<string, unknown>) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (typeof snapshot.agreedPrice === "number" && snapshot.currency === "THB") tx.create(ref(db, "priceHistory", `sale-${saleId}`), { birdId, amount: snapshot.agreedPrice, currency: snapshot.currency, effectiveOn: createdOn, kind: "final", sourceType: "sale", saleId, createdAt: now() });
