...
const externalOrigins = new Set(["farm_hatched", "purchased", "external", "unknown"]);
...
export const createExternalBird = async (db: Firestore, input: Record<string, unknown>): Promise<{ birdId: string; ringId: string }> => {
  ...
  if (typeof input.origin !== "string" || !externalOrigins.has(input.origin)) fail("invalid-argument", "origin must be farm_hatched, purchased, external, or unknown.");
