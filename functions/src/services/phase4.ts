...
export const createDelivery=async(db:Firestore,input:Record<string,unknown>)=>{const saleId=requireId(input.saleId,"saleId"),createdOn=requireDate(input.createdOn,"createdOn");const distanceKm=nonNegative(input.distanceKm,"distanceKm"),freeDistanceKm=nonNegative(input.freeDistanceKm,"freeDistanceKm"),pricePerKm=nonNegative(input.pricePerKm,"pricePerKm"),shippingFee=Math.max(distanceKm-freeDistanceKm,0)*pricePerKm;return db.runTransaction(async tx=>{const[sale,completedHandovers]=await Promise.all([tx.get(ref(db,"sales",saleId)),tx.get(db.collection("handovers").where("saleId","==",saleId).where("status","==","completed"))]);if(!sale.exists)fail("not-found","Sale not found.");if(sale.data()?.status!=="completed")fail("failed-precondition","Delivery requires a completed sale.");if(!completedHandovers.empty)fail("failed-precondition","Delivery cannot be created after completed handover.");const deliveryId=id();tx.create(ref(db,"deliveries",deliveryId),{saleId,distanceKm,freeDistanceKm,pricePerKm,shippingFee,currency:requireId(input.currency,"currency"),createdOn,status:"planned",createdAt:now(),updatedAt:now()});return{deliveryId};});};
