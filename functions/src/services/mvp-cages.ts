...
export const createExternalBirdInCageMvp = async (db: Firestore, input: Record<string, unknown>) => {
  ...
  const origin = enumValue(input.origin, "origin", ["purchased", "external", "unknown"]); const acquiredOn = input.acquiredOn ? requireDate(input.acquiredOn, "acquiredOn") : undefined; const mutation = optionalText(input.mutation);
