...
export function PriceHistory({ birdId, onSaved }: { birdId: string; onSaved: () => Promise<void> }) {
  ...
  return <section className="history-ledger" aria-label="ประวัติราคา">
    ...
    {hasSaleFinal ? <p className="sales-truth-note">มีราคาสุดท้ายจากการขายแล้ว ไม่สามารถเพิ่มราคาตั้งขายหรือราคาที่เสนอได้</p> : <form className="card" onSubmit={submit}>
      ...
      <label className="field">ประเภท *<select aria-label="ประเภทราคา" value={kind} onChange={event => setKind(event.target.value)}><option value="list">ราคาตั้งขาย</option><option value="offer">ราคาที่เสนอ</option></select></label>
