...
export function OperatorGate() {
  ...
  if (operator?.isOperator) return <ThaiUiLabels><header className="operator-bar"><span>ผู้ใช้งาน: {operator.displayName || operator.email || operator.uid}</span><button type="button" onClick={signOut}>ออกจากระบบ</button></header>{error && <p role="alert">{error}</p>}<App /></ThaiUiLabels>;
