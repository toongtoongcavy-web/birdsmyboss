...
export function PassportWorkflow({ birds, handovers, onRefresh }: { birds: Row[]; handovers: Row[]; onRefresh: () => Promise<void> }) {
  ...
  if (selected && detail) return <section className="passport-control passport-control-detail">
    ...
    <section className={`passport-control-identity ${birdVisualClass(detail.status)}`}><OrangeRing variant="selected"/><div><h3>{displayValue(detail.displayName)}</h3><p>Ring ID: {displayValue(detail.ringId)} · {displayValue(detail.mutation)}</p><p>วันฟัก/วันเกิด: {isoToThaiDisplay(detail.hatchedOn) || "-"} · {displayOrigin(detail.origin)}</p></div></section>
